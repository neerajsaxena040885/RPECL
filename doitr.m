...
% Start preparing error components now ...
if NEC>0
   PrepareEC
end
...
disp('Creating Random Parameter Draws.');
% Preparing the Master Draws matrix (MASTERDR) of dimensions NV x NP x MASTERDRAWS
MASTERDR=makedraws;   
...
if NV>0 & NF>0
   param=[F;B(IDV(:,2)~=5,1);W;EC];
elseif NV>0 & NF==0
   param=[B(IDV(:,2)~=5,1);W;EC];
elseif NV==0 & NF>0;
   param=[F;EC];
else
...
if NEC>0
  echat=paramhat(NF+NV+NV+1,1);
  ecsd=stderr(NF+NV+NV+1,1);
end
...
disp('              ------------------ ');
disp('                Est         SE ');
for r=1:length(NAMESEC);
    fprintf('%-10s %10.4f %10.4f\n', NAMESEC{r,1}, [echat(r,1) ecsd(r,1)]);
end
disp(' ');
end
