...
% Array of draws has dimension MASTERDRAWS x NP x (NALTMAX-1).
...
% (NALTMAX-1)]. Call this as the Master Draws matrix 
% -----------------------------------------------------------------

function dr=ECHYmakedraws
 
global NV NP DRAWTYPE MASTERDRAWS NALTMAX NCSMAX

% Define an empty array to store the draws
dr=zeros(MASTERDRAWS,NP,NCSMAX,(NALTMAX-1));
   ...
   for j=1:(NALTMAX-1)
       ...
       while test == 0
            ...
            for m=1:(hh-1);
                ...
                test=size(draws,1) >= ((NP.*NCSMAX.*MASTERDRAWS) + 10);    % +10 here means that we are generating extra 10 draws, which means I would be dropping first 10 elements in next steps. This is being done in order to minimise correlation 
                if test == 1
                   break
                end
            end
       ...
       draws=draws(11:(NP.*NCSMAX.*MASTERDRAWS)+10,1);   % Dropping the first 10 elements from further processing. This is being done in order to minimise correlation among prime numbers
       ...
       if DRAWTYPE == 3     % Shifted and Shuffled Halton Draws
            ...
            draws=reshape(draws,MASTERDRAWS,NP,NCSMAX);
            ...
            draws=reshape(draws,NP.*NCSMAX.*MASTERDRAWS,1);  % a column array draws having NP*MASTERDRAWS rows
       end
       % Since Error Components are Standard Normally distributed.
       draws=-sqrt(2).*erfcinv(2.*draws);  %Take inverse cum normal
       dr(:,:,:,j)=reshape(draws,MASTERDRAWS,NP,NCSMAX);  % a 3-D matrix of shifted and shuffled Halton draws
    end
end
